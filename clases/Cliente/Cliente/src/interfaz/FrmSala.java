...
public class FrmSala extends javax.swing.JFrame implements PnlObservador, IActualizable {
    ...
    private void actualizarMarcador(Marcador marcador) {
        ...
        for (int i = 0; i < this.sala.getMarcador().getJugadores().size(); i++) {
            switch (i) {
                case 0:
                    total+=marcador.getJugadores().get(i).getPuntaje();
        ...
        if(total==22){
            ...
            FrmFinJuego fin=new FrmFinJuego(this.sala.getMarcador().getJugadores(), this.jugador);
            fin.setVisible(true);
            this.dispose();
        }
    }
        ...
        if (mensaje instanceof Marcador) {
            ...
            for (int i = 0; i < marcador.getJugadores().size(); i++) {
                if (marcador.getJugadores().indexOf(this.jugador) == marcador.getSiguiente()) {
                    ...
                    this.pnlTablero.actualizaTurno(false);
                }
            }
                ...
                for (Jugador jugador : this.sala.getMarcador().getJugadores()) {
                    if (jugador.equals(formas.get(i).getJugador())) {
                        ...
                        if (i == 0) {
                            ...
                            int p=this.sala.getMarcador().getJugadores().get(index).getPuntaje();
                            this.sala.getMarcador().getJugadores().get(index).setPuntaje(p+1);
                            System.out.println("Marcador actualizado por segunda vez");
                            this.actualizarMarcador(this.sala.getMarcador());
                        }
                    }
                }
            }
